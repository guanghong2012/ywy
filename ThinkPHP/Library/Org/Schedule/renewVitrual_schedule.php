...
class renewVitrual_schedule implements schedule {
    public function exec($data){
        set_time_limit(0);
        //续费虚拟主机
        $Api = new \Common\Api\CloundApi();

        $return = $Api->renewVitrual($data);
        ...
        if($return['code']==200 && !empty($return['data'])){
            $this->updateVitrual($data['goods_id'],$return['data']);
        }
        return $result;
    ...
    //更新用户虚拟机到期时间
    public function updateVitrual($order_goods_id,$returndata)
    {
        ...
        //返回参数未知 ...
        //更新到期时间
        $origin_expire_time = M('user_vitrual')->where('id='.$goods['user_goods_id'])->getField('expire_time');
        $new_expire_time = strtotime($origin_expire_time) + $buy_config['Timeperiod']*3600*24*30;//新的到期时间
        $expire_time = $returndata['expiry_time'] ? $returndata['expiry_time'] : date('Y-m-d H:i:s',$new_expire_time);
        M('user_vitrual')->where('id='.$goods['user_goods_id'])->setField('expire_time',$expire_time);
