<!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="UTF-8">
	<meta name="keywords" content="wsg,hth">
	<meta name="description" content="ASW">       
	<meta http-equiv="X-UA-Compatible" content="IE=EmulateIE9" />
	<meta http-equiv="X-UA-Compatible" content="IE=edge" />   
	<meta name="keywords" content="亿维云网">
	<Meta name="description" Content="亿维云网">
	<title>亿维云网</title>
	<link href="css/css.css" rel="stylesheet" type="text/css" />
	<script src="js/jquery.js"></script><!-- 统一调用js -->
	<script src="js/SuperSlide/jquery.superslide.2.1.1.js"></script><!-- 切换 -->
	<link href="css/style2.css" rel="stylesheet" type="text/css" /><!-- 表单美化 -->
</head>
<body>
	<?php require_once('top.php'); ?>
	<div class="virtual_lx_div">
		<div class="w1200 mbs_div">
			<ul>
				<li><a href="index.php">首页</a></li>
				<li>></li>
				<li><a href="virtual.php">虚拟主机</a></li>
				<li>></li>
				<li><a href="virtual_lx.php">基础型空间</a></li>
				<li>></li>
				<li class="li1">基础型空间 - 飓风2（S）</li>
			</ul>
			<div class="clear"></div>
		</div>
		<div class="vir_xq w1200">
			<h1>虚拟主机型号: 飓风2(S)</h1>
			<h2>基本信息</h2>
			<div class="div1">
				<ul>
					<li class="li1">
						<p><input name="radio-1-set" id="checkbox-1-2" class="regular-checkbox" type="radio"><label class="label_1" for="checkbox-1-2"></label><span>电信<em>240.00元/年</em></span></p>
						<p><input name="radio-1-set" id="checkbox-1-3" class="regular-checkbox" type="radio"><label class="label_1" for="checkbox-1-3"></label><span>双线<em>240.00元/年</em></span></p>
						<p><input name="radio-1-set" id="checkbox-1-4" class="regular-checkbox" type="radio"><label class="label_1" for="checkbox-1-4"></label><span>三线<em>240.00元/年</em></span></p>
					</li>
					<li class="li2">
						<p><span>操作系统：</span>Windows和Linux平台任选</p>
						<p><span>Web空间：</span>200M</p>
						<p><span>支持：</span>独立IP</p>
					</li>
					<li class="li2">
						<p><span>邮局个数：</span>5个</p>
					</li>
				</ul>
				<div class="clear"></div>
			</div>
			<div class="div2">
				<p><input id="checkbox-1-1" class="regular-checkbox" checked="" type="checkbox"><label class="label_1" for="checkbox-1-1"></label>增加独立IP  <span>（增加独立IP可让您的网站快速收录, 可减少IP被封机率）</span>
				<a href="">立即购买</a>
				</p>
				<div class="clear"></div>
			</div>
			<h2>详细信息</h2>
			<div class="div3">
				<h3>Windows系统支持功能/组件详情：</h3>
				<table width="100%" border="0" cellspacing="1" cellpadding="3" bgcolor="#e5e5e5" class="ta1">
				  <tr>
				    <td width="25%" bgcolor="#f2f2f2">Access数据库</td>
				    <td width="25%" bgcolor="#f2f2f2">Asp脚本语言</td>
				    <td width="25%" bgcolor="#f2f2f2">FSO组件</td>
				    <td width="25%" bgcolor="#f2f2f2">Jmail组件</td>
				  </tr>
				  <tr>
				    <td bgcolor="#f2f2f2">AspUpload组件</td>
				    <td bgcolor="#f2f2f2">AspJpeg组件</td>
				    <td bgcolor="#f2f2f2">动易组件</td>
				    <td bgcolor="#f2f2f2"></td>
				  </tr>
				</table>
			</div>
			<div class="div3">
				<h3>Linux系统支持功能/组件详情：</h3>
				<table width="100%" border="0" cellspacing="1" cellpadding="3" bgcolor="#e5e5e5" class="ta1">
				  <tr>
				    <td width="25%" bgcolor="#f2f2f2">MySQL数据库空间：<span>30M</span></td>
				    <td width="25%" bgcolor="#f2f2f2">MySQL数据库个数：<span>1个</span></td>
				    <td width="25%" bgcolor="#f2f2f2">phpMyAdmin管理面板</td>
				    <td width="25%" bgcolor="#f2f2f2">PHP脚本语言</td>
				  </tr>
				  <tr>
				    <td bgcolor="#f2f2f2">Zend加密程序</td>
				    <td bgcolor="#f2f2f2">支持Sqlite数据库</td>
				    <td bgcolor="#f2f2f2"></td>
				    <td bgcolor="#f2f2f2"></td>
				  </tr>
				</table>
			</div>
			<div class="div3">
				<h3>其它及注意事项：</h3>
				<table width="100%" border="0" cellspacing="1" cellpadding="3" bgcolor="#e5e5e5" class="ta1">
				  <tr>
				    <td width="25%" bgcolor="#f2f2f2">月流量限制：<span>20000M/月</span></td>
				    <td width="25%" bgcolor="#f2f2f2">并发连接数：<span>150个</span></td>
				    <td width="25%" bgcolor="#f2f2f2">免费三级域名</td>
				    <td width="25%" bgcolor="#f2f2f2">主机控制面板</td>
				  </tr>
				  <tr>
				    <td bgcolor="#f2f2f2">购买实时开通</td>
				    <td bgcolor="#f2f2f2">数据定期备份</td>
				    <td bgcolor="#f2f2f2">千兆防火墙</td>
				    <td bgcolor="#f2f2f2">不间断电源：<br><span>双路市电+UPS+后备发电机</span></td>
				  </tr>
				</table>
			</div>
			<div class="div4">
				<h3><span>技术支持：</span> 7×24小时在线提问系统及客服热线：020-66849155 66849156 66849157</h3>
			</div>
		</div>
	</div>
	
	<?php require_once('bottom.php'); ?>
</body>
</html>